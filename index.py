...
# ====== THIẾT LẬP BAN ĐẦU ======
setup_complete = 0
setup_complete_lock = threading.Lock()
setup_complete_event = asyncio.Event()
main_loop = None
worker_queues = []
worker_queues_lock = threading.Lock()
profiles_list = Utility.read_data('profile_name')
class Setup:
...
class Auto:
    ...
    def _run(self):
        ...
        with setup_complete_lock:
            setup_complete += 1
        # Check xem đã là profile cuối cùng chưa
        if setup_complete >= len(profiles_list):
            self.node.log("Tất cả profiles đã setup xong.")
            Utility.print_section(f"Bot đã sẵn sàng quiz","✅")
            main_loop.call_soon_threadsafe(setup_complete_event.set)
        # Bắt đầu chờ task
        while True:
            try:
                ...
                if answer_task is None:
                    ...
                    Utility.wait_time(1)
                    break
                self.node.log(f"Nhận đáp án {answer_task} ")
...
class Discord:
    ...
    async def handle_quiz(question, author = None, description = None):
        if description:
            desc_lines = description.strip().splitlines()
            desc_final = "\n".join(desc_lines[:4])
            print(f"\n{author}\n{question}\n{description}\n")
            answer = await Discord.ask_gpt(question, desc_final)
            if answer:
            ...
            print(f"\n -> Quizz: \n{question}\n")
            answer = await Discord.ask_gpt(question)
            if answer:
                print(f"** Đáp án: --> {answer}")
                task_to_send = answer[0]
                #print(f"Task to send: {task_to_send}")
                with worker_queues_lock:
                    for queue in worker_queues:
                        queue.put(task_to_send)
            else:
    ...
    async def ask_gpt(question, description = ""):
        ...
        search_data = await Discord.search_serper(question_final)
        if search_data:
...
class Selenium:
    def __init__(self) -> None:
        ...
        if not self.profiles:
            ...
            exit()
        self.browserMgr = BrowserManager(AutoHandlerClass=Auto, SetupHandlerClass=Setup)
        self.run_terminal(
    ...
    def run_terminal_wrapper(self, loop, event, **args):
        """
        ...
        try:
            ...
            self.quiz_worker()  # Bắt đầu worker chờ task
    ...
    def run_terminal(self, profiles: list[dict], max_concurrent_profiles: int = 4, auto: bool = False, sys_chrome: bool = False):
            '''
            ...
            print(f"   📍 Đường dẫn Profiles:   {self.browserMgr.user_data_dir}")
            print("=" * 70+"\n")

            while is_run:
                user_data_profiles = []

                if self.browserMgr.user_data_dir.exists() and self.browserMgr.user_data_dir.is_dir():
                    raw_user_data_profiles = [folder.name for folder in self.browserMgr.user_data_dir.iterdir() if folder.is_dir()]
                ...
                if choice in ('1', '2', '3'):
                    ...
                    if choice == '1':
                        ...
                        self.browserMgr.run_stop(selected_profiles)
                        Utility.print_section("Kết thúc cấu hình","✅")                
                    
                    elif choice == '2':
                        Utility.print_section("Khởi động trình duyệt","🔄")                
                        self.browserMgr.run_multi(profiles=selected_profiles,
                        ...
                        for profile_name in selected_profiles:
                            ...
                            if not isinstance(profile_name, str):
                                continue
                            profile_path = self.browserMgr.user_data_dir / profile_name
                            try:
                    ...
                    Utility.print_section('LỖI: Lựa chọn không hợp lệ. Vui lòng thử lại...', "🛑")
    # Hàm chờ task
    '''def quiz_worker(self):
        print("Worker quiz đã sẵn sàng chờ task.")
        while True:
            try:
...
# ====== KHỞI TẠO DISCORD BOT VÀ LẮNG NGHE TIN NHẮN ======
...
async def on_message(message): # Lắng nghe msg từ discord và lọc lấy msg cần dùng.
    ...
    if message.channel.id == CHANNEL_ID:
        if message.author.id == BOT_ID or message.author.bot:
            if message.content:
                if Discord.is_quiz_message(message.content, "Contents"):
                    await Discord.handle_quiz(message)
            if message.embeds:
                for embed in message.embeds:
                    if Discord.is_quiz_message(embed.title, "Embeds"):
                        await Discord.handle_quiz(embed.title, embed.author, embed.description)
    if message.channel.id == 1401580731225866312 and message.author.id == 1161783017245773864 and "shutdown" in message.content.lower():
        print(f"[SYSTEM] Shutdown message received.\n\n[Browser] Shutting down...")
        with worker_queues_lock:
            for queue in worker_queues:
                queue.put(None)
    elif message.channel.id == 1401580731225866312 and message.author.id == 1161783017245773864:
                    print(f"[SYSMaster] {message.author.global_name} đã nói: {message.content}")

worker_thread = threading.Thread(target=lambda: Selenium(), daemon=True)
